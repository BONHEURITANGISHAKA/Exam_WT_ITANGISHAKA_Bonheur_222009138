...
<main>
<div style="background-color: #fff; padding: 20px; border-radius: 5px; box-shadow: 0 0 10px rgba(0, 0, 0, 0.1); width: 300px; margin: 20px auto;">    <h2 style="text-align: center; margin-bottom: 20px;">User Form</h2>
    
    <form id="userForm" method="post" action="" style="display: flex; flex-direction: column;">
...
<table border="1" style="width: 100%; border-collapse: collapse; color: white;">
    ...
    if ($result->num_rows > 0) {
        ...
        while($row = $result->fetch_assoc()) {
            ...
            echo "<td style='padding: 8px; border: 1px solid #ddd;'>" . $row["Password"] . "</td>";
            echo "<td style='padding: 8px; border: 1px solid #ddd;'><a href='?update=" . $row["ID"] . "' style='text-decoration: none; color: blue;'>Update</a> | <a href='user.php?delete=" . $row["ID"] . "' onclick='return confirm(\"Are you sure you want to delete this record?\")' style='text-decoration: none; color: red;'>Delete</a></td>";
            echo "</tr>";
        }
    } else {
...
if (isset($_GET['update'])) {
    ...
    $update_sql = "SELECT * FROM users WHERE ID = '$update_id'";
...
<div style="background-color: #fff; padding: 20px; border-radius: 5px; box-shadow: 0 0 10px rgba(0, 0, 0, 0.1); width: 300px; margin: 20px auto;">
    ...
    <h2 style="text-align: center; margin-bottom: 20px;">Update User</h2>

...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST['update'])) {
        ...
        $question = $conn->real_escape_string($_POST['qwestion']);
        $duration = $conn->real_escape_string($_POST['duration']);

        // SQL query to update data in the quiz table
        $sql = "UPDATE qwiz SET Name='$name', Description='$description', qwestion='$question', Duration='$duration' WHERE ID='$id'";
        ...
        $name = $conn->real_escape_string($_POST['name']);
        $description = $conn->real_escape_string($_POST['description']);
        $qwestion = $conn->real_escape_string($_POST['qwestion']);
        $duration = $conn->real_escape_string($_POST['duration']);

        // SQL query to insert data into the quiz table
        $insert_sql = "INSERT INTO qwiz (Name, Description, qwestion, Duration) VALUES ('$name', '$description', '$qwestion', '$duration')";
...
if (isset($_GET['delete'])) {
    ...
    // SQL query to delete record from quiz table
    $delete_sql = "DELETE FROM qwiz WHERE ID = '$delete_id'";
...
<header>
    <nav>
        ...
        <ul style="list-style: none; margin: 0; padding: 0; display: flex; background-color: #333;">
            ...
            <li style="margin-left: 20px;"><a href="Qwiz.php" style="text-decoration: none; color: white; padding: 10px; display: block;">Qwiz</a></li>
...
<div style="background-color: #fff; padding: 20px; border-radius: 5px; box-shadow: 0 0 10px rgba(0, 0, 0, 0.1); width: 300px; margin: 20px auto;">
    ...
    <h2 style="text-align: center; margin-bottom: 20px;">Quiz Form</h2>
    
    <form id="quizForm" method="post" action="" style="display: flex; flex-direction: column;">
        ...
        <label for="qwestion" style="margin-bottom: 5px;">Question:</label>
        <input type="text" id="question" name="question" required style="padding: 10px; margin-bottom: 10px; border-radius: 5px; border: 1px solid #ccc;">

        <label for="duration" style="margin-bottom: 5px;">Duration:</label>
        <input type="text" id="duration" name="duration" required style="padding: 10px; margin-bottom: 10px; border-radius: 5px; border: 1px solid #ccc;">
...
<table border="1" style="width: 100%; border-collapse: collapse;">
    ...
    if ($result->num_rows > 0) {
        ...
        while($row = $result->fetch_assoc()) {
            ...
            echo "<td style='padding: 8px; border: 1px solid #ddd; color: white;'>" . $row["Qwestion"] . "</td>";
            echo "<td style='padding: 8px; border: 1px solid #ddd; color: white;'>" . $row["Duration"] . "</td>";
            echo "<td style='padding: 8px; border: 1px solid #ddd;'><a href='?update=" . $row["ID"] . "' style='text-decoration: none; color: blue;'>Update</a> | <a href='Qwiz.php?delete=" . $row["ID"] . "' onclick='return confirm(\"Are you sure you want to delete this record?\")' style='text-decoration: none; color: red;'>Delete</a></td>";
            echo "</tr>";
        }
    } else {
        echo "<tr><td colspan='5' style='padding: 8px; border: 1px solid #ddd;'>0 results</td>";
    }
    ?>
</table>
...
if (isset($_GET['update'])) {
    ...
    $update_sql = "SELECT * FROM qwiz WHERE ID = '$update_id'";
...
<div style="background-color: #fff; padding: 20px; border-radius: 5px; box-shadow: 0 0 10px rgba(0, 0, 0, 0.1); width: 300px; margin: 20px auto;">
    ...
    <h2 style="text-align: center; margin-bottom: 20px;">Update Quiz</h2>
    ...
    <form id="updateForm" method="post" action="" style="display: flex; flex-direction: column;">
        ...
        <label for="qwestion" style="margin-bottom: 5px;">Question:</label>
        <input type="text" id="qwestion" name="qwestion" required style="padding: 10px; margin-bottom: 10px; border-radius: 5px; border: 1px solid #ccc;" value="<?php echo $update_row['Qwestion']; ?>">
        <label for="duration" style="margin-bottom: 5px;">Duration:</label>
        <input type="text" id="duration" name="duration" required style="padding: 10px; margin-bottom: 10px; border-radius: 5px; border: 1px solid #ccc;" value="<?php echo $update_row['Duration']; ?>">

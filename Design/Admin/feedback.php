...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST['update'])) {
        ...
        // SQL query to update data in the feedback table
        $sql = "UPDATE feedback SET Comments='$comments', Timestamp='$timestamp' WHERE ID='$id'";
        ...
        $insert_sql = "INSERT INTO feedback (Comments, Timestamp) VALUES ('$comments', '$timestamp')";
...
if (isset($_GET['delete'])) {
    ...
    // SQL query to delete record from feedback table
...
<div style="background-color: #fff; padding: 20px; border-radius: 5px; box-shadow: 0 0 10px rgba(0, 0, 0, 0.1); width: 300px; margin: 20px auto;">
    ...
    <form id="feedbackForm" method="post" action="" style="display: flex; flex-direction: column;">

        <label for="comments" style="margin-bottom: 5px;">Comments:</label>
        <input type="text" id="comments" name="comments" required style="padding: 10px; margin-bottom: 10px; border-radius: 5px; border: 1px solid #ccc;">
...
<table border="1" style="width: 100%; border-collapse: collapse;">
    ...
    if ($result->num_rows > 0) {
        ...
        while($row = $result->fetch_assoc()) {
            echo "<tr>";
            echo "<td style='padding: 8px; border: 1px solid #ddd; color: white;'>" . $row["Comments"] . "</td>";
            echo "<td style='padding: 8px; border: 1px solid #ddd; color: white;'>" . $row["Timestamp"] . "</td>";
            echo "<td style='padding: 8px; border: 1px solid #ddd;'><a href='?update=" . $row["ID"] . "' style='text-decoration: none; color: blue;'>Update</a> | <a href='Feedback.php?delete=" . $row["ID"] . "' onclick='return confirm(\"Are you sure you want to delete this record?\")' style='text-decoration: none; color: red;'>Delete</a></td>";
            echo "</tr>";
        }
    } else {
...
if (isset($_GET['update'])) {
    ...
    $update_sql = "SELECT * FROM feedback WHERE ID = '$update_id'";
...
<div style="background-color: #fff; padding: 20px; border-radius: 5px; box-shadow: 0 0 10px rgba(0, 0, 0, 0.1); width: 300px; margin: 20px auto;">
    ...
    <h2 style="text-align: center; margin-bottom: 20px;">Update Feedback</h2>
    ...
    <form id="updateForm" method="post" action="" style="display: flex; flex-direction: column;">
        ...
        <label for="comments" style="margin-bottom: 5px;">Comments:</label>
        <input type="text" id="comments" name="comments" required style="padding: 10px; margin-bottom: 10px; border-radius: 5px; border: 1px solid #ccc;" value="<?php echo $update_row['Comments']; ?>">
        <label for="timestamp" style="margin-bottom: 5px;">Timestamp:</label>
